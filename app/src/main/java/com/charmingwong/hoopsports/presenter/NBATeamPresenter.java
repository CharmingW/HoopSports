...
public class NBATeamPresenter implements IPresenter {
    private final String DATA_TAG = "nba_regular_data";
    private final String TAG = "NBATeamPresenter";
    private final String CACHE_DIR = "nba_regular_data";
    private String mTeamUrl = "http://op.juhe.cn/onebox/basketball/team?key=1ddf89489dd1e0e52b270236897c63c7&";
    private String mCombatUrl = "http://op.juhe.cn/onebox/basketball/combat?key=1ddf89489dd1e0e52b270236897c63c7&";
    private String mUrl;

    private OnResponseCallback mOnResponseCallback;

    private NBATeamParser mDataParser;

    private Context mContext;

    private static NBATeamPresenter mInstance;

    public void setTeam(String host, String guest) {
        StringBuilder stringBuilder;
        if (guest != null) {
            stringBuilder = new StringBuilder(mCombatUrl);
            stringBuilder.append("hteam=");
            stringBuilder.append(host);
            stringBuilder.append("&vteam=");
            stringBuilder.append(guest);
        } else {
            stringBuilder = new StringBuilder(mTeamUrl);
            stringBuilder.append("team=");
            stringBuilder.append(host);
        }
        mUrl = stringBuilder.toString();
        Log.i(TAG, "requestData: " + stringBuilder.toString());
    }

    private NBATeamPresenter(Context context) {
        mContext = context;
    }

    public static NBATeamPresenter getInstance(Context context) {
        if (mInstance == null) {
            synchronized (NBATeamPresenter.class) {
                if (mInstance == null) {
                    mInstance = new NBATeamPresenter(context);
                }
            }
        }
        return mInstance;
